...
cl::opt<bool> EnableHyFMPA("func-merging-hyfm-pa", cl::init(false), cl::Hidden,
...
static unsigned
RandomLinearizationOfBlocks(Function &F,
                            SmallVectorImpl<BasicBlock *> &OrederedBBs) {
  std::set<BasicBlock *> Visited;
...
void FunctionMerger::orderBlocks(Function &F,
                                 SmallVectorImpl<BasicBlock *> &OrderedBBs,
...
Timer TimeAlignRank("Merge::CodeGen::Align::Rank",
    ...
    if (ExplorationThreshold == 1) {
      ...
      if (best_match.candidate != nullptr)
        if (!EnableF3M || best_match.Distance < RankingDistance)
          ...
          EstimateThunkOverhead(it->candidate, best_match->candidate)) // Needs
          AlwaysPreserved return;
          }*/
...
public:
  ...
  void print_stats() override {
    ...
    errs() << "STATS: Avg Duplicate Hashes: "
           << (1.0 * duplicate_hashes) / candidates.size() << "\n";
    for (size_t i = 0; i < 20; i++)
      errs() << "STATS: Histogram Bucket Size " << (1 << i) << " : "
             << hist_bucket_size[i] << "\n";
    for (size_t i = 0; i < 21; i++)
      errs() << "STATS: Histogram Distances " << i * 0.05 << " : "
             << hist_distances[i] << "\n";
    for (size_t i = 0; i < 21; i++)
      errs() << "STATS: Histogram Distances Diff " << i * 0.05 << " : "
             << hist_distances_diff[i] << "\n";
  }
...
void FunctionMerger::extendAlignedSeq(AlignedSequence<Value *> &AlignedSeq,
...
FunctionMerger::merge(Function *F1, Function *F2, std::string Name,
  ...
  PtrToRefContainer<BasicBlock, SmallVector<BasicBlock *, 8>> F1RefBlocks(
      F1Blocks);
  PtrToRefContainer<BasicBlock, SmallVector<BasicBlock *, 8>> F2RefBlocks(
      F2Blocks);

  AlignmentStats TotalAlignmentStats;
  AlignedSequence<Value *> AlignedSeq = alignBlocks(
      F1RefBlocks, F2RefBlocks, TotalAlignmentStats, nullptr, Options);
  ...
  */
  ...
  ////TODO: merging attributes might create compilation issues if we are not
  ///careful. /Therefore, attributes are not being merged right now.
  // auto AttrList1 = F1->getAttributes();
  // auto AttrList2 = F2->getAttributes();
...
/*static int EstimateThunkOverhead(Function* F1, Function* F2,
                                 StringSet<> &AlwaysPreserved) {
  int fParams = F1->getFunctionType()->getNumParams() +
F2->getFunctionType()->getNumParams(); return RequiresOriginalInterfaces(F1, F2,
AlwaysPreserved) * (2 + fParams);
}*/
...
bool FunctionMerging::runImpl(
  ...
  while (matcher->size() > 0) {
    ...
    while (!Rank.empty()) {
      ...
      errs() << F1Name << " + " << F2Name << " <= " << Name
             << " Tries: " << MergingTrialsCount << " Valid: " << match.Valid
             << " BinSizes: " << match.OtherSize << " + " << match.Size
             << " <= " << match.MergedSize
      ...
      errs() << " TotalTime: "
             << (time_iteration_end - time_ranking_start) / 1us
             << " RankingTime: "
             << (time_ranking_end - time_ranking_start) / 1us
             << " AlignTime: " << (time_align_end - time_align_start) / 1us
             << " CodegenTime: "
             << ((time_codegen_end - time_codegen_start) -
                 (time_align_end - time_align_start)) /
                    1us
  ...
  errs() << "Timer:CodeGen:Total: "
         << TimeCodeGenTotal.getTotalTime().getWallTime() << "\n";
  TimeCodeGenTotal.clear();
  ...
  errs() << "Timer:CodeGen:Align:Rank: "
         << TimeAlignRank.getTotalTime().getWallTime() << "\n";
  TimeAlignRank.clear();
  ...
  errs() << "Timer:CodeGen:PostOpt: "
         << TimePostOpt.getTotalTime().getWallTime() << "\n";
  TimePostOpt.clear();
...
static void CodeGen(FunctionMerger::CodeGenerator *CG, BlockListType &Blocks1,
                    BlockListType &Blocks2, BasicBlock *EntryBB1,
          ...
          for (Instruction &I : *BB) {
            ...
            if (HasBeenMerged) {
              ...
              if (LastMergedBB) {
                ...
                // << " with " << NodeBB->getName() << "\n";
                bool check = ChainBlocks(LastMergedBB, NodeBB, IsFunc1);
                if (!check) {
                  return false;
                }
              } else {
              ...
              if (LastMergedBB) {
                ...
                NewBB = BasicBlock::Create(MergedFunc->getContext(), BBName,
                                           MergedFunc);
                bool check = ChainBlocks(LastMergedBB, NewBB, IsFunc1);
                if (!check) {
                  return false;
                }
                BlocksFX[NewBB] = BB;
...
bool FunctionMerger::SALSSACodeGen::generate(
  ...
  CodeGen(this, Blocks1, Blocks2, EntryBB1, EntryBB2, MergedFunc, IsFunc1,
          PreBB, AlignedSeq, VMap, BlocksF1, BlocksF2, MaterialNodes);
  ...
  for (auto &Entry : AlignedSeq) {
    ...
    if (I1 != nullptr && I2 != nullptr) {
      ...
      // if (I1 != nullptr) {
      //   auto *NewI = dyn_cast<Instruction>(VMap[I1]);
      //   NewI->setDebugLoc(DebugLoc());
      // }
      // if (I2 != nullptr) {
      //   auto *NewI = dyn_cast<Instruction>(VMap[I2]);
      //   NewI->setDebugLoc(DebugLoc());
      // }
...
void FunctionMerger::SALSSACodeGen::StoreInstIntoAddr(Instruction *IV,
                                                      Value *Addr) {
...
AllocaInst *
FunctionMerger::SALSSACodeGen::MemfyInst(std::set<Instruction *> &InstSet) {
  BasicBlock *PreBB = CodeGenerator::getPreBlock();
  if (InstSet.empty())
    return nullptr;
